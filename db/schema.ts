...
// Each token can be claimed by only 1 user
export const tokenPool = pgTable('token_pool', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  value: text('value').notNull().unique(),
  claimCount: integer('claim_count').notNull().default(0), // Track how many times this token has been claimed (max 1)
  assignedTo: uuid('assigned_to').references(() => keys.id), // User who currently owns this token
  assignedAt: timestamp('assigned_at', { withTimezone: true }), // When the token was assigned
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().default(sql`now()`),
}, (table) => {
  return {
    valueIdx: unique('token_value_unique_idx').on(table.value),
    availableTokensIdx: index('available_tokens_idx').on(table.claimCount).where(sql`claim_count < 1`),
  }
})

// Deliveries - audit log of token distribution
// Each delivery references a unique token (1 user per token)
export const deliveries = pgTable('deliveries', {
  ...
  return {
    keyTokenIdx: unique('delivery_key_token_idx').on(table.keyId, table.tokenId),
    tokenUniqueIdx: unique('delivery_token_unique_idx').on(table.tokenId), // Ensure each token is delivered to only one user
  }
})
