...
describe('API Token Tests', () => {
  ...
  it('should not allow token sharing between 2 users', async () => {
    // Create a new token
    const sharedTokenValue = 'TEST-TOKEN-NOT-SHARED-XYZA1234567890'
    ...
    // Second user should NOT be able to claim the same token
    ...
    if (response2.status === 200) {
      ...
      // Second user should get a DIFFERENT token (or no tokens available)
      expect(secondToken).not.toBe(sharedTokenValue)
    }
  })
